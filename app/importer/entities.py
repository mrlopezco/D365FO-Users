"""OData POST helpers for employees, system users, and person links."""
...
from app.config.entity import build_entity_rows
...
from app.util.helpers import resolve_company, user_label
    ...
    for index, (user, row_values) in enumerate(zip(users, rows, strict=True), start=1):
        ...
        if not party:
            if dry_run:
                ...
                preview = json.dumps(payload, indent=2, default=str)
                print(
                    f"  [dry-run] sample PersonUsers payload for {label} "
                    f"(PartyNumber placeholder):"
                )
                for line in preview.splitlines():
                    print(f"    {line}")
                print(f"  [{index}/{total}] {label} — dry-run (skipped POST)")
                ...
                continue
            print(
                f"  [{index}/{total}] {label} — skipped link "
                f"(no PartyNumber for UserId {user_id!r})",
                file=sys.stderr,
            )
            result.failed += 1
