...
def print_preflight_report(report: PreflightReport) -> None:
    ...
    print()
    print("Preflight (OData GET — existing records in environment)")
    print(f"  Input rows: {total}")

    if not report.has_issues:
        print(f"  No duplicates found. All {total} row(s) look new for user and employee checks.")
        print()
        return

    print(
        f"  System users already present: {report.system_user_duplicate_count} "
        f"(create will be skipped; not blocking)"
    )
    print(
        f"  Employees already present: {report.employee_duplicate_count} "
        f"(F&O may allow another worker — review before proceeding)"
    )
    print()

    for row in report.rows:
        if not row.has_any_issue:
            continue
        print(f"  • {row.label}")
        for reason in row.system_user_matches:
            print(f"      System user: {reason}")
        for reason in row.employee_matches:
            print(f"      Employee:    {reason}")
        if row.person_link_note:
            print(f"      Link:        {row.person_link_note}")
    print()
