...
def _resolve_company(env: D365Environment, config: dict[str, Any]) -> str | None:
    if env.company:
        return env.company
...
def _import_assignment_batch(
    *,
    ...
    for index, assignment in enumerate(assignments, start=1):
        ...
        row_values = _resolve_security_row(columns, row_dict)
...
def _resolve_security_row(columns: dict[str, Any], record: dict[str, str]) -> list[Any]:
    values: list[Any] = []
